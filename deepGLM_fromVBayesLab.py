...
#     rqmc = np.arange(S*(d_theta+1)).reshape([S, d_theta+1])/100 ### Test random ###############################
    ...
    for s in range(S):
        ...
        if (distr == 'normal'):    
            ...
            grad_llh,yNN = nnGradLogLikelihood(W_seq,beta,X,y,datasize,distr,mean_sigma2_inverse) #원래는 mean_sigma2_inverse 없었음(사실 계산에 필요없긴함)
        
        grad_h = grad_prior_w_beta+grad_llh    # Gradient of log prior plus log-likelihood
        grad_log_q = vbGradientLogq(b,c,theta,mu,isotropic) #normal pdf log버전
...
#         minibatch = np.copy(data)
        ...
        if iter % 1 == 0:
            ...
            if isotropic:
                for j in range(p):
                    mean_column_j_tilde[j] = dot(mu_matrixW1_tilde[:,j].transpose(),mu_matrixW1_tilde[:,j])\
                                                ...
                                                + c^2*n_units[0]
                    mu_tau[j] = shrinkage_gamma[j]/sqrt(mean_column_j_tilde[j]) #식(21) (mu_tau = 1/tau^2의 inverse gaussian dist의 mean
                    lambda_tau[j] = shrinkage_gamma[j]^2 #식(21) (mu_tau = 1/tau^2의 inverse gaussian dist의 shape
            else:
                c_matrixW1_tilde = my_reshape(c[W1_tilde_index-1],n_units[0],p)
                for j in range(p):
            ...
            mean_tau = 1/mu_tau + 1/lambda_tau
            shrinkage_gamma = sqrt((n_units[0] + 1)/mean_tau) #식(22) small gamma update 
...
#design matrix with ones
designMatWithOne_data = np.c_[np.ones((1000,1)), Xmat_data, TPSmat_data]
designMatWithOne_cv = np.c_[np.ones((500,1)), Xmat_cv, TPSmat_cv]
# print(designMatWithOne_data.shape) # 1000, 103



# data = loadmat("Sample Code/abaloneBART.mat")
# data = loadmat("C:\gitProject\SpaBasisML\sampleCode\DirectMarketing.mat")
# X = data['X']
# X_test = data['X_test']
# y = toVector(data['y'])
